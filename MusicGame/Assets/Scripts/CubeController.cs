...
public class CubeController : MonoBehaviour {
    ...
    */
        ...
        {
            transform.Translate(Vector3.left * moveVar * Time.deltaTime * InputDevice.DPadLeft);
            directionFace = false;
        }
        else if(InputManager.ActiveDevice.DPadRight.IsPressed)
        {
            transform.Translate(Vector3.right * moveVar * Time.deltaTime * InputDevice.DPadRight);
            directionFace = true;
        }
    ...
    */
    ...
    void LateUpdate()
    {
        ...
        Camera.main.transform.position = new Vector3 (transform.position.x, 4, transform.position.z-10);
    }
    ...
    IEnumerator dash(float dashDur) {
        ...
        while (dashDur > time) //we call this loop every frame while our custom boostDuration is a higher value than the "time" variable in this coroutine
        {
            ...
            {
                transform.Translate(Vector3.right * moveVar * Time.deltaTime * boostSpd);
            }
            else
            {
                transform.Translate(Vector3.left * moveVar * Time.deltaTime * boostSpd);
            }
